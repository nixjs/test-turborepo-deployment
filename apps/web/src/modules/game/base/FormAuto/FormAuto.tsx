...
import { GameAutoPlayTypes, RareType } from './types'
...
interface FormAutoPropArg {
    playConfig: GameAutoPlayTypes.FormAutoPlayConfig
    isPlaying: boolean
    autoPlay: boolean
    tokenSelected: string
    onSetPlayConfig: (params: GameAutoPlayTypes.FormAutoPlayConfig) => void
}

export const FormAuto: React.FC<FormAutoPropArg> = React.memo(({ playConfig, autoPlay, isPlaying, tokenSelected, onSetPlayConfig }) => {
    const onWinType = Number(playConfig[FormState.ON_WIN_TYPE as GameAutoPlayTypes.AutoLayConfigKeys])
    const onLossType = Number(playConfig[FormState.ON_LOSS_TYPE as GameAutoPlayTypes.AutoLayConfigKeys])
    ...
    const onChange = (e: InputNumberValue | React.ChangeEvent<HTMLInputElement>, type: 'Customize' | 'Native') => {
        if (e) {
            ...
            switch (name) {
                ...
                default: {
                    payload = { ...playConfig, [name as GameAutoPlayTypes.AutoLayConfigKeys]: value }
                    break
                }
            }
            if (typeof onSetPlayConfig === 'function') onSetPlayConfig(payload)
        }
    }

    const infinityIcon = React.useMemo(
        () =>
            (playConfig && +playConfig[FormState.NUMBER_OF_BET as GameAutoPlayTypes.AutoLayConfigKeys] === 0 && (
                ...
                <Col className="d-flex flex-md-column" md={4}>
                    <FormGroup label="Number of bets" htmlFor={FormState.NUMBER_OF_BET} className="mb-16" contentClass="p-0">
                        <InputNumber
                            ...
                            value={String(playConfig[FormState.NUMBER_OF_BET as GameAutoPlayTypes.AutoLayConfigKeys])}
                    ...
                    <FormGroup label="Max bet amount" htmlFor={FormState.MAX_BET_AMOUNT} contentClass="p-0">
                        <InputNumber
                            ...
                            value={String(playConfig[FormState.MAX_BET_AMOUNT as GameAutoPlayTypes.AutoLayConfigKeys])}
                            min={0}
                            prefixClass="mr-8"
                ...
                <Col className="d-flex flex-md-column" md={4}>
                    <FormGroup label="Stop on profit" htmlFor={FormState.STOP_ON_PROFIT} className="mb-16" contentClass="p-0">
                        <InputNumber
                            ...
                            value={String(playConfig[FormState.STOP_ON_PROFIT as GameAutoPlayTypes.AutoLayConfigKeys])}
                            prefixHTML={<Symbol source={`/tokens/ic24px_${tokenSelected.toLowerCase() || 'none'}.svg`} />}
                            suffixHTML={<span>{tokenSelected}</span>}
                            min={0}
                            prefixClass="mr-8"
                        ...
                        <div className="d-flex align-items-center">
                            ...
                            <div className="ml-10 flex-fill">
                                <InputNumber
                                    ...
                                    value={String(playConfig[FormState.ON_WIN_VALUE as GameAutoPlayTypes.AutoLayConfigKeys])}
                ...
                <Col className="d-flex flex-md-column" md={4}>
                    <FormGroup label="Stop on loss" htmlFor={FormState.STOP_ON_LOSS} className="mb-16" contentClass="p-0">
                        <InputNumber
                            ...
                            value={String(playConfig[FormState.STOP_ON_LOSS as GameAutoPlayTypes.AutoLayConfigKeys])}
                            prefixHTML={<Symbol source={`/tokens/ic24px_${tokenSelected.toLowerCase() || 'none'}.svg`} />}
                            suffixHTML={<span>{tokenSelected}</span>}
                            min={0}
                            prefixClass="mr-8"
                        ...
                        <div className="d-flex align-items-center">
                            ...
                            <div className="ml-10 flex-fill">
                                <InputNumber
                                    ...
                                    value={String(playConfig[FormState.ON_LOSS_VALUE as GameAutoPlayTypes.AutoLayConfigKeys])}
                                    suffixHTML={<span>%</span>}
                                    min={0}
                                    prefixClass="mr-8"
                                    placeholder="On lose"
