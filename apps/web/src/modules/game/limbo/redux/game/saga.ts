...
import { MessageBrokerKey } from 'modules/game/limbo/consts/enum'
...
import { GameRequest } from 'modules/game/limbo/grpc/request'
import { InHouseRequestMethods } from 'modules/game/limbo/grpc/request/methods'
    ...
    try {
        while (true) {
            ...
            const { data, status }: Interfaces.ResponseData<GameBaseTypes.Turn> = yield call(
                ...
                inhouseInstance,
                {
                    methodName: InHouseRequestMethods.LIMBO_CREATE_NEW_TURN,
                    params: new pbjs.Empty()
                },
                false
            )
            if (status === 'SUCCESS' && data) {
                MB.messageBroker.publish(MessageBrokerKey.LIMBO_CREATE_NEW_TURN, data)
            }
        }
    } catch (error) {
    ...
    try {
        while (true) {
            ...
            const { data, status, error }: Interfaces.ResponseData<GameBaseTypes.PlaceBetReply> = yield call(
                ...
                inhouseInstance,
                {
                    methodName: InHouseRequestMethods.LIMBO_PLACE_BET,
                    params: GameRequest.placeBetRequestParams(payload)
                },
                false
            )
            if (status === 'SUCCESS' && data) {
                MB.messageBroker.publish(MessageBrokerKey.LIMBO_PLACE_BET_RESULT, data)
            } else {
                MB.messageBroker.publish(MessageBrokerKey.LIMBO_PLACE_BET_ERROR, error)
            }
        }
    } catch (error) {
